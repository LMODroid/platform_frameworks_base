...
public class ThemeOverlayController extends SystemUI {
    ...
    static final String OVERLAY_LMODROID_BLACK_THEME =
    ...
    private void applyBlackTheme(boolean state) {
        ...
        try {
            mOverlayManager.setEnabled(OVERLAY_LMODROID_BLACK_THEME, state, userId);
            if (DEBUG) {
                Log.d(TAG, "applyBlackTheme: overlayPackage="
                        + OVERLAY_LMODROID_BLACK_THEME + " userId=" + userId);
            }
        } catch (Exception e) {
            Log.e(TAG, "Failed to " + (state ? "enable" : "disable")
                    + " overlay " + OVERLAY_LMODROID_BLACK_THEME + " for user " + userId);
        }
    }
