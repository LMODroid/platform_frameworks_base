...
class PulseLightNotifManager @Inject constructor(
    ...
    private fun setupTuner() {
        val tunable = Tunable { key: String?, value: String? ->
            when (key) {
                PULSE_AMBIENT_LIGHT -> {
                    enabled = TunerService.parseIntegerSwitch(value, false)
                }
                PULSE_AMBIENT_LIGHT_DURATION -> {
                    value?.toIntOrNull()?.let { duration = it }
                }
                PULSE_AMBIENT_LIGHT_REPEAT_COUNT -> {
                    ...
                    value?.toIntOrNull()?.let { lightCount = 1 + it }
                }
            }
        }
    ...
    companion object {
        ...
        private const val PULSE_AMBIENT_LIGHT_DURATION = Settings.Secure.PULSE_AMBIENT_LIGHT_DURATION
        private const val PULSE_AMBIENT_LIGHT_REPEAT_COUNT = Settings.Secure.PULSE_AMBIENT_LIGHT_REPEAT_COUNT
    }
